...
import { ChevronRight, Instagram, Menu, Search, ShoppingCart, Star, User } from 'lucide-react';
...
export default function Header() {
  ...
  return (
    ...
    >
      <div className={cn('absolute inset-0 z-[-1] transition-opacity duration-300', isSolid ? 'opacity-100 bg-background border-b border-border/50' : 'opacity-0')} />
      ...
      <div className="container flex h-16 items-center px-4 sm:px-6 lg:px-10 mx-auto w-full md:w-[80%] relative z-10">
        <div className="flex items-center flex-1 md:flex-none md:justify-start">
          <Link href="/" className="flex items-center space-x-2">
            <span className={cn('font-medium tracking-[0.15em] sm:tracking-[0.2em] md:tracking-[0.3em] uppercase whitespace-nowrap transition-colors duration-300 text-sm sm:text-base md:text-xl', textColor)}>
              Glitters Shop
            </span>
          </Link>
        </div>
        ...
        <nav className="hidden md:flex items-center justify-center flex-1 px-4" onMouseLeave={handleMouseLeave}>
          <ul className="flex items-center justify-center gap-8 h-full">
            {menuData.map((item) => (
              <li key={item.id} className="relative h-16 flex items-center" onMouseEnter={() => handleMouseEnter(item.id)}>
                <Link
                  href={`/shop?category=${item.id}`}
                  className={cn('text-sm tracking-widest uppercase transition-all duration-300 py-1 relative', activeMenu === item.id ? 'font-bold' : 'font-medium hover:font-bold', textColor)}
                >
                  {item.label}
                  <span className={cn('absolute -bottom-1 left-0 w-full h-[2px] origin-left bg-current transition-transform duration-300 ease-out', activeMenu === item.id ? 'scale-x-100' : 'scale-x-0')} />
                </Link>
              </li>
            ))}
          </ul>
        ...
        <div className="flex items-center justify-end md:gap-x-1">
          ...
          <Button variant="ghost" size="icon" asChild className={cn('transition-colors duration-300', iconColorWrapper)}>
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[400px] bg-background/80 backdrop-blur-xl border-l border-border/50 p-0 flex flex-col h-full z-50">
              <SheetHeader className="p-6 border-b border-border/10">
                <SheetTitle className="text-xl font-bold tracking-tight">Menú</SheetTitle>
              </SheetHeader>
                  ...
                  <nav className="flex flex-col space-y-1">
                    {mobileTiendaLinks.map((link) => (
                      <SheetClose asChild key={link.label}>
                        <Link href={link.href} className="group flex items-center justify-between py-2 text-lg font-medium transition-all hover:pl-2 hover:text-primary">
                          {link.label}
            ...
            <div className="container mx-auto px-8 py-10 w-full md:w-[80%]">
              <div className="grid grid-cols-12 gap-10">
                <motion.div variants={childVariants} className="col-span-3 border-r border-border/50 pr-8">
                  ...
                  <Link href={`/shop?category=${activeData.id}`} className="inline-flex items-center text-sm font-bold text-primary hover:text-primary/80 transition-colors group">
                    ...
                    <motion.div key={idx} variants={childVariants} className="space-y-5">
                      ...
                      <ul className="space-y-3">
                        {col.items.map((link, linkIdx) => (
                          <li key={linkIdx}>
                            <Link href={link.href} className="group flex items-center text-sm text-foreground/80 hover:text-primary transition-colors font-medium">
                ...
                <motion.div variants={childVariants} className="col-span-3 pl-4 border-l border-border/50">
                  <Link href={`/shop?category=${activeData.id}`} className="block relative aspect-[4/5] w-full overflow-hidden rounded-md group">
                    <Image src={activeData.image.src} alt={activeData.image.alt} fill className="object-cover transition-transform duration-700 group-hover:scale-105" />
