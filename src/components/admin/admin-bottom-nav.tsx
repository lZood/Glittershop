...
import { BarChart3, Layers, LayoutDashboard, Package, ShoppingBag } from 'lucide-react';
            ...
            {navItems.map((item) => {
                ...
                return (
                    <Link
                        key={item.href}
                        href={item.href}
                        className={cn('relative flex flex-col items-center gap-1.5 p-1 transition-all duration-300 min-w-[60px] active:scale-95 group')}
                    >
                        {isActive && (
                            <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-8 h-[2px] bg-brand shadow-[0_0_8px_rgba(180,115,49,0.8)]" />
                        )}
