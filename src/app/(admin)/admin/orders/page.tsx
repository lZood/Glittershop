...
import { Eye, Filter, MoreHorizontal, Search, Truck } from 'lucide-react';
        ...
        async function fetchOrders() {
            ...
            if (result.success && result.data) {
                const formattedOrders = (result.data as any[]).map((o) => {
                    ...
                    const customerName = parsedAddress?.full_name || o.guest_email || 'Cliente Desconocido';
            ...
            <div className="space-y-6">
                {loading ? (
                    ...
                    filteredOrders.map((order) => (
                        <Card
                            key={order.realId}
                            className="border border-border/50 shadow-none rounded-none bg-background hover:border-border transition-colors group"
                        >
                            <div className="p-6 md:p-8">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <Badge
                                                variant="outline"
                                                className={cn(
                                                    'rounded-none px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.1em]',
                                ...
                                <div className="mb-8">
                                    <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground block mb-2">
                                        Comprador
                                    </span>
                                ...
                                <div className="flex items-center gap-4">
                                    <Link href={`/admin/orders/${order.realId}`} className="flex-1">
                                        {order.status === 'Enviado' ? (
                                            <Button
                                                variant="outline"
                                                className="w-full rounded-none bg-background hover:bg-secondary text-foreground font-semibold uppercase tracking-[0.1em] h-12 text-xs"
                                            >
                                    ...
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <Button
                                                variant="outline"
                                                size="icon"
                                                className="h-12 w-12 rounded-none bg-background text-foreground hover:bg-secondary hover:text-foreground"
                                            >
                                                <MoreHorizontal className="w-5 h-5" />
                                            </Button>
                                        </DropdownMenuTrigger>
                                        <DropdownMenuContent align="end" className="rounded-none">
                                            <DropdownMenuItem className="text-xs tracking-[0.05em] uppercase font-medium">
                                                Ver Detalles Completos
                                            </DropdownMenuItem>
                                            <DropdownMenuItem className="text-xs tracking-[0.05em] uppercase font-medium">
                                                Contactar Cliente
                                            </DropdownMenuItem>
                                            <DropdownMenuItem className="text-xs tracking-[0.05em] uppercase font-medium text-red-600">
                                                Cancelar Pedido
                                            </DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
